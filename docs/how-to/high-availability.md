...
## Changing SQL Server version or edition

The SQL Server version and edition are controlled by the **Docker image** and the `MSSQL_PID` environment variable in the StatefulSet.

### Change the version

Update the `image` field in the StatefulSet:
...
Add `MSSQL_PID` to the environment variables:
...
Par défaut (sans `MSSQL_PID`), l'image utilise **Developer Edition**.
...
Le rolling update redémarre un pod à la fois. L'AG auto-failover gère la bascule automatiquement pendant le redémarrage du primary.

### Vérifier la version et l'édition
...
Ou via le CRD `SQLServer` si vous en avez un :
